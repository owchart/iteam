...
namespace OwLib
{
    ...
    /// Gdi+绘图类
    /// </summary>
    public class GdiPlusPaint : CPaint
    {
        ...
        public virtual void AddArc(RECT rect, float startAngle, float sweepAngle)
        {
        ...
        public virtual void AddBezier(POINT[] points)
        {
        ...
        /// </summary>
        /// <param name="points">点阵</param>
        public virtual void AddCurve(POINT[] points)
        {
            Point[] gdiPlusPoints = new Point[points.Length];
            for (int i = 0; i < gdiPlusPoints.Length; i++)
            {
        ...
        public virtual void AddEllipse(RECT rect)
        {
            int rw = rect.right - rect.left - 1;
            if (rw < 1) rw = 1;
            int rh = rect.bottom - rect.top - 1;
        ...
        public virtual void AddRect(RECT rect)
        {
            int rw = rect.right - rect.left - 1;
            if (rw < 1) rw = 1;
            int rh = rect.bottom - rect.top - 1;
        ...
        /// <param name="startAngle">从 x 轴到弧线的起始点沿顺时针方向度量的角（以度为单位）</param>
        /// <param name="sweepAngle">从 startAngle 参数到弧线的结束点沿顺时针方向度量的角（以度为单位）</param>
        public virtual void AddPie(RECT rect, float startAngle, float sweepAngle)
        {
            int rw = rect.right - rect.left - 1;
            if (rw < 1) rw = 1;
            int rh = rect.bottom - rect.top - 1;
        ...
        /// <param name="rect">区域</param>
        public virtual void AddText(String text, FONT font, RECT rect)
        {
            ...
            if (m_scaleFactorX != 1 || m_scaleFactorY != 1)
            {
                ...
                FONT scaleFont = new FONT(font.m_fontFamily, fontSize, font.m_bold, font.m_underline, font.m_italic);
        ...
        /// <param name="startAngle">从 x 轴到弧线的起始点沿顺时针方向度量的角（以度为单位）</param>
        /// <param name="sweepAngle">从 startAngle 参数到弧线的结束点沿顺时针方向度量的角（以度为单位）</param>
        public virtual void DrawArc(long dwPenColor, float width, int style, RECT rect, float startAngle, float sweepAngle)
        {
        ...
        public void DrawBezier(long dwPenColor, float width, int style, POINT[] points)
        {
            if (dwPenColor == COLOR.EMPTY) return;
            Point[] gdiPlusPoints = new Point[points.Length];
            for (int i = 0; i < gdiPlusPoints.Length; i++)
        ...
        public virtual void DrawCurve(long dwPenColor, float width, int style, POINT[] points)
        {
            if (dwPenColor == COLOR.EMPTY) return;
            Point[] gdiPlusPoints = new Point[points.Length];
            for (int i = 0; i < gdiPlusPoints.Length; i++)
            ...
            else
            {
                ...
                if (File.Exists(file))
                {
                    ...
                    if (values[2] != null)
                    {
                        ...
                        {
                            source[i] = CStr.ConvertStrToInt(strs[i]);
                        }
                    ...
                    if (image != null)
                    {
                        long highColor = COLOR.EMPTY, lowColor = COLOR.EMPTY;
                        ...
                        if (highColor != COLOR.EMPTY && lowColor != COLOR.EMPTY)
                        {
                            int A = 0, R = 0, G = 0, B = 0;
                            COLOR.ToARGB(this, highColor, ref A, ref R, ref G, ref B);
                            Color gdiPlusHighColor = Color.FromArgb(A, R, G, B);
                            COLOR.ToARGB(this, lowColor, ref A, ref R, ref G, ref B);
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < strs.Length; i++)
                            {
                                corners[i] = CStr.ConvertStrToInt(strs[i]);
                            }
                            int left = 0;
                            int top = 0;
                            int right = iw;
        ...
        /// <param name="startAngle">从 x 轴到弧线的起始点沿顺时针方向度量的角（以度为单位）</param>
        /// <param name="sweepAngle">从 startAngle 参数到弧线的结束点沿顺时针方向度量的角（以度为单位）</param>
        public virtual void DrawPie(long dwPenColor, float width, int style, RECT rect, float startAngle, float sweepAngle)
        {
            if (dwPenColor == COLOR.EMPTY) return;
            int rw = rect.right - rect.left - 1;
            if (rw < 1) rw = 1;
        ...
        public virtual void DrawRoundRect(long dwPenColor, float width, int style, RECT rect, int cornerRadius)
        {
            if (dwPenColor == COLOR.EMPTY) return;
            int rw = rect.right - rect.left - 1;
            if (rw < 1) rw = 1;
        ...
        public virtual void FillGradientEllipse(long dwFirst, long dwSecond, RECT rect, int angle)
        {
            int rw = rect.right - rect.left;
            if (rw < 1) rw = 1;
            int rh = rect.bottom - rect.top;
        ...
        /// <param name="angle">角度</param>
        public virtual void FillGradientPath(long dwFirst, long dwSecond, RECT rect, int angle)
        {
            int rw = rect.right - rect.left;
            if (rw < 1) rw = 1;
            int rh = rect.bottom - rect.top;
        ...
        /// <param name="angle">角度</param>
        public virtual void FillGradientPolygon(long dwFirst, long dwSecond, POINT[] points, int angle)
        {
        ...
        /// <param name="cornerRadius">边角半径</param>
        /// <param name="angle">角度</param>
        public virtual void FillGradientRect(long dwFirst, long dwSecond, RECT rect, int cornerRadius, int angle)
        {
            int rw = rect.right - rect.left;
            if (rw < 1) rw = 1;
            int rh = rect.bottom - rect.top;
        ...
        /// <param name="startAngle">从 x 轴到弧线的起始点沿顺时针方向度量的角（以度为单位）</param>
        /// <param name="sweepAngle">从 startAngle 参数到弧线的结束点沿顺时针方向度量的角（以度为单位）</param>
        public virtual void FillPie(long dwPenColor, RECT rect, float startAngle, float sweepAngle)
        {
            if (dwPenColor == COLOR.EMPTY) return;
            int rw = rect.right - rect.left - 1;
            if (rw < 1) rw = 1;
        ...
        /// <param name="rect">矩形区域</param>
        /// <param name="cornerRadius">边角半径</param>
        public virtual void FillRoundRect(long dwPenColor, RECT rect, int cornerRadius)
        {
            if (dwPenColor == COLOR.EMPTY) return;
            int rw = rect.right - rect.left - 1;
            if (rw < 1) rw = 1;
        ...
        protected SolidBrush GetBrush(long dwPenColor)
        {
            ...
            if (m_brush == null)
            {
                ...
                {
                    m_brushColor = COLOR.EMPTY;
                }
            }
            else
            {
                if (m_brushColor == COLOR.EMPTY || m_brushColor != dwPenColor)
                {
        ...
        public virtual long GetColor(long dwPenColor)
        {
            ...
            {
                if (dwPenColor == COLOR.CONTROL)
                {
                    dwPenColor = 16777215;
                }
                else if (dwPenColor == COLOR.CONTROLBORDER)
                {
                    dwPenColor = 3289650;
                }
                else if (dwPenColor == COLOR.CONTROLTEXT)
                {
                    dwPenColor = 0;
                }
                else if (dwPenColor == COLOR.DISABLEDCONTROL)
                {
                    dwPenColor = 13158600;
                }
                else if (dwPenColor == COLOR.DISABLEDCONTROLTEXT)
                {
                    dwPenColor = 3289650;
                }
                else if (dwPenColor == COLOR.HOVEREDCONTROL)
                {
                    dwPenColor = 13158600;
                }
                else if (dwPenColor == COLOR.PUSHEDCONTROL)
                {
                    dwPenColor = 9868950;
                }
            }
        ...
        protected Font GetFont(FONT font)
        {
            if (font.m_strikeout)
            {
        ...
        protected Pen GetPen(long dwPenColor, float width, int style)
        {
            ...
            {
                if (m_penColor == COLOR.EMPTY || m_penColor != dwPenColor)
                {
        ...
        public virtual POINT Rotate(POINT op, POINT mp, int angle)
        {
        ...
        public virtual void SetOffset(POINT mp)
        {
            m_offsetX = mp.x;
            m_offsetY = mp.y;
        }
        ...
        public virtual SIZEF TextSizeF(String text, FONT font)
        {
            ...
            return new SIZEF(gdiPlusSize.Width, gdiPlusSize.Height);
        }
        #endregion
    }
}